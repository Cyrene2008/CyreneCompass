<template>
  <div ref="shellRef" class="app-shell" :class="[{ dark: settings.dark }, `${mode}-mode`]" :style="shellStyle" @contextmenu.prevent @pointermove="noteActivity" @pointerdown="noteActivity" @keydown="noteActivity">
    <div v-if="mode === 'ball'" class="ball-wrap mode-surface">
      <button class="floating-ball" :class="{ 'image-ball': settings.ballStyle === 'image', 'text-ball': settings.ballStyle === 'text' }" :style="ballStyle" :aria-label="t('settings')" @contextmenu.prevent @pointerdown.prevent="onBallDown" @pointermove.prevent="onBallMove" @pointerup.prevent="onBallUp" @pointercancel.prevent="onBallCancel">
    ...
    <FloatingImageCropper v-if="cropSource" :source="cropSource" :title="t('cropBallImage')" :zoom-label="t('cropZoom')" :cancel-label="t('cancel')" :confirm-label="t('confirm')" @cancel="cropSource = ''" @confirm="saveCroppedBallImage" />
...
import { dataUrlByteLength, resolveBallRadius } from './utils/floatingBallStyle'
...
const version = CURRENT_VERSION; const mode = ref('ball'); const section = ref('general'); const breadcrumb = ref([]); const editBreadcrumb = ref([]); const toast = ref(''); const gridRef = ref(); const shellRef = ref(); const iconPickerItem = ref(null); const iconSearch = ref(''); const exitStage = ref(0); const systemAccent = ref('#0078d4'); const cropSource = ref(''); let idleTimer; let ballPointer; let centerPointer; let titlebarPointer; let transitioning = false; const cleanups = []
...
if (saved && saved.settingsVersion !== 5) {const usesLegacyDefaultBall=(saved.ballStyle??'solid')==='solid'&&(saved.ballColor??DEFAULT_ACCENT).toLowerCase()===DEFAULT_ACCENT; const ballStyle=usesLegacyDefaultBall?'text':(saved.ballStyle??'text'); saved = { ...saved, settingsVersion:5, uiScale:saved.uiScale??100, iconSize:saved.iconSize??42, fontSize:saved.fontSize??15, ballRadius:saved.ballRadius??(ballStyle==='image'?(saved.ballImageCrop?50:0):50), ballStyle, ballText:saved.ballText??'罗盘' }}
const settings = ref({ settingsVersion:5,language:'zh',theme:'peach',accent:DEFAULT_ACCENT,dark:false,ballStyle:'text',ballText:'罗盘',ballColor:DEFAULT_ACCENT,ballSize:72,ballOpacity:86,compassOpacity:100,compassSize:560,uiScale:100,iconSize:42,fontSize:15,fontFamily:'HarmonyOS',ballImage:'/cyrene.png',ballRadius:50,autoHide:true,autoHideSeconds:10,autoStart:false,registryStart:false,items:baseItems(),...saved })
...
const ballStyle = computed(() => ({width:`${settings.value.ballSize}px`,height:`${settings.value.ballSize}px`,opacity:settings.value.ballOpacity/100,background:settings.value.ballStyle==='image'?'transparent':settings.value.ballColor,'--ball-radius':`${resolveBallRadius(settings.value.ballStyle,settings.value.ballRadius)}%`,'--ball-text-size':`${Math.max(12,settings.value.ballSize*(ballTextLines.value.length>1?.235:.285))}px`}))
...
async function chooseBallImage(){const path=await open({multiple:false,filters:[{name:'Images',extensions:['png','jpg','jpeg','webp']} ]});if(!path)return;try{const source=await invoke('read_visual_data_url',{path});if(dataUrlByteLength(source)>12*1024*1024){showToast(t('imageTooLarge'));return}cropSource.value=source}catch(e){showToast(String(e))}}
function saveCroppedBallImage(source){settings.value.ballImage=source;settings.value.ballStyle='image';cropSource.value='';persist()}
